...
/* ─────────────────────────────────────────────
   PASTE INTO: src/routes/history.tsx
───────────────────────────────────────────── */
...
/* ── Daily quiz helpers — resets each calendar day ── */
  ...
  if (stage === "outcome" && chosen) {
    return (
      <div className="rounded-3xl border border-white/20 bg-white/8 p-5">
        ...
        <button onClick={() => setStage("history")} className="w-full py-4 rounded-full font-black text-white" style={{ background: "oklch(0.72 0.18 350)" }}>
      ...
      <div className="space-y-2 mb-5">
        {items.map((item, i) => (
          <div key={item.id} draggable
            ...
            style={{ borderColor: checked && !isCorrect && item.correctIndex !== i ? "oklch(0.65 0.22 25)" : "rgba(255,255,255,0.15)", background: "rgba(255,255,255,0.06)" }}>
            <span className="text-white/30 text-sm w-5 shrink-0 text-center font-mono">{i + 1}</span>
            <span className="text-white flex-1 leading-snug font-semibold text-sm">{item.text}</span>
            <span className="text-white/20 text-lg shrink-0">⠿</span>
          </div>
        ))}
      </div>
      {checked && !isCorrect && (
        <p className="text-white/50 text-sm mb-3 text-center">Not quite — red-bordered items are out of place. Try again.</p>
      )}
      <button onClick={checked && !isCorrect ? () => { setChecked(false); setIsCorrect(null); } : checkOrder}
...
/* ══════════════════════════════════════════════
   QUIZ GAME — real MCQ, one at a time, continue button
══════════════════════════════════════════════ */
type QuizState = {
  currentIndex: number;
...
function QuizGame({ questions, onComplete, onXpGain }: {
  questions: Question[];
  onComplete: (correct: number, total: number) => void;
  onXpGain: (amount: number) => void;
}) {
  const [state, setState] = useState<QuizState>({
    currentIndex: 0, selectedOption: null, hasAnswered: false, correctCount: 0, isComplete: false,
  });
  ...
  const handleSelect = useCallback((optionIndex: number) => {
    ...
    setState((prev) => ({
      ...prev, selectedOption: optionIndex, hasAnswered: true,
  ...
  const handleNext = () => {
    ...
    if (nextIndex >= questions.length) {
      ...
      setState((prev) => ({ ...prev, currentIndex: nextIndex, selectedOption: null, hasAnswered: false }));
    }
  };
  ...
  if (state.isComplete) {
    ...
    return (
      <div className="rounded-3xl border border-white/20 bg-white/8 p-5 text-center">
        ...
        <h3 className="font-black text-2xl mb-1">{state.correctCount} / {questions.length}</h3>
        <p className="text-white/60 font-bold mb-5">{pct >= 80 ? "Excellent work!" : pct >= 60 ? "Well done!" : "Keep learning!"}</p>
        <div className="w-full bg-white/10 rounded-full h-2.5 mb-2">
          <div className="h-2.5 rounded-full transition-all duration-700"
            style={{ width: `${pct}%`, background: pct >= 80 ? "oklch(0.55 0.18 145)" : pct >= 60 ? "oklch(0.65 0.18 80)" : "oklch(0.65 0.22 25)" }} />
        </div>
    ...
    <div>
      ...
      <div className="flex flex-col gap-3 mb-4">
        {q.options.map((opt, idx) => {
          ...
          if (show) {
            if (isCorrectOption) cls += "border-green-500 bg-green-500/20 text-green-300";
      ...
      {/* Continue button — fixed above nav, appears after answering */}
      {state.hasAnswered && (
        <div className="fixed right-0 bottom-20 left-0 z-10 px-6">
          <button onClick={handleNext} className="w-full py-4 rounded-full font-black text-white shadow-lg"
            style={{ background: "oklch(0.72 0.18 350)" }}>
            {state.currentIndex + 1 >= questions.length ? "See My Results →" : "Next Question →"}
          </button>
        </div>
      )}
    </div>
...
function HistoryPage() {
  ...
  useEffect(() => {
    try { setDailyDone(localStorage.getItem(getDailyStorageKey(chapter.id)) === "done"); } catch { /* ok */ }
  }, [chapter.id]);
  ...
  const handleDecideDone = () => { markDone("decide"); addXp(chapter.xpReward); setChaptersCompleted((c) => c + 1); };
  ...
  return (
    <main className="relative min-h-screen overflow-y-auto bg-background px-6 pt-10 pb-32 text-white">
      ...
      <div className="relative mb-2">
        ...
        <p className="text-white/40 text-sm">7 chapters · 1788–1799 · Swipe timeline to explore</p>
      </div>
      ...
      <div className="relative flex gap-2 flex-wrap mb-5">
        {chapters.map((c, i) => {
          ...
          return (
            <button key={c.id} onClick={() => { if (unlocked) { setChapterIndex(i); setActiveTab("learn"); } }}
              className={`px-3 py-1.5 rounded-full text-sm font-black transition-all ${
                i === chapterIndex ? "text-white" : unlocked ? "bg-white/10 text-white" : "bg-white/5 text-white/25 cursor-not-allowed"
              }`}
              style={i === chapterIndex ? { background: "oklch(0.72 0.18 350)" } : {}}>
              {done ? "✓ " : !unlocked ? "🔒" : ""}{c.id}
            </button>
          );
        })}
      ...
      <div className="relative rounded-3xl border border-white/20 bg-white/8 p-4 mb-5">
        <p className="text-xs font-black text-white/40 uppercase tracking-widest mb-1">{chapter.date} · {chapter.keyFigure}</p>
        ...
        {tabs.map((t) => (
          <button key={t.id} onClick={() => !t.locked && setActiveTab(t.id)}
            className={`flex-1 py-2.5 rounded-full text-xs font-black transition-all ${
              activeTab === t.id ? "text-white" : t.locked ? "bg-white/5 text-white/20 cursor-not-allowed" : "bg-white/10 text-white"
            }`}
            style={activeTab === t.id ? { background: "oklch(0.72 0.18 350)" } : {}}>
      ...
      {activeTab === "learn" && (
        <div className="relative">
          <div className="flex justify-between items-center mb-5">
            <span className="text-xs font-black px-3 py-1.5 rounded-full text-white" style={{ background: "oklch(0.72 0.18 350)" }}>
              {difficultyLabel(difficulty)} content
            </span>
            <span className="text-white/40 text-xs font-bold">{learnContent.length} sections</span>
          </div>

          <div className="space-y-5 mb-8">
            {learnContent.map((text, i) => (
              <div key={i} className="rounded-2xl border border-white/10 bg-white/5 p-5">
                <p className="text-white/85 leading-relaxed text-sm font-semibold">{text}</p>
              </div>
            ))}
          </div>

          <div className="text-white/40 text-xs font-bold mb-4">XP: {xp} · Tier: {difficultyLabel(difficulty)}</div>

          {!doneTabs.has("learn") ? (
            <button onClick={handleLearnDone} className="w-full py-4 rounded-full font-black text-white"
              style={{ background: "oklch(0.72 0.18 350)" }}>
              Done Reading → Unlock Quiz (+20 XP)
            </button>
          ) : (
            <p className="text-center text-white/40 font-bold text-sm">✓ Learn complete — Quiz tab is unlocked</p>
          )}
        </div>
      )}
        ...
        <div className="relative">
          {!doneTabs.has("quiz") ? (
            <>
              <div className="flex justify-between items-center mb-5">
                <p className="text-white/60 font-bold text-sm">
                  ...
                  <span style={{ color: "oklch(0.78 0.18 350)" }}>{difficultyLabel(difficulty)}</span>
                </p>
                <span className="text-xs bg-white/10 px-3 py-1.5 rounded-full text-white/60 font-black">Daily Quiz</span>
              </div>
              <QuizGame
                ...
                onXpGain={addXp}
              />
              {/* Spacer so fixed continue button doesn't overlap content */}
              <div className="h-20" />
            </>
          ) : (
            <div className="text-center py-10">
              ...
              <p className="text-white/40 text-sm mb-6">New questions available tomorrow.</p>
              <p className="text-white/50 font-bold">
                Head to <span className="text-white font-black">Order</span> tab to continue →
              </p>
            </div>
          )}
        </div>
          ...
          {!doneTabs.has("order") ? (
            ...
            <div className="text-center py-10">
              ...
              <p className="text-white/50 font-bold mt-1">
                Head to <span className="text-white font-black">Decide</span> →
              </p>
            </div>
          )}
        </div>
      ...
      {activeTab === "decide" && (
        <div className="relative">
          {!doneTabs.has("decide") ? (
            ...
            <div className="text-center py-10">
              <div className="text-5xl mb-3">🏆</div>
              <p className="font-black text-xl mb-1">Chapter {chapter.id} Complete!</p>
              <p className="text-white/60 font-bold mb-6">+{chapter.xpReward} XP earned</p>
              {chapterIndex < chapters.length - 1 ? (
                ...
                <p className="text-white/40 font-bold">You've completed all 7 chapters! 🎉</p>
              )}
            </div>
          )}
        </div>
