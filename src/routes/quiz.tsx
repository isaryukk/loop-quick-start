...
const FALLBACK: Question[] = [
  {
    ...
    correctAnswer: 1,
    explanation: "NATO is a military alliance formed in 1949 between North American and European nations for collective defence.",
    topic: "Politics",
  },
  {
  ...
  },
  {
    ...
    correctAnswer: 0,
    explanation: "GDP measures the total monetary value of all goods and services produced in a country.",
    topic: "Economics",
  },
  {
    ...
    options: ["3", "5", "7", "10"],
    correctAnswer: 1,
    explanation: "Five permanent members hold veto power: USA, UK, France, Russia, and China.",
    topic: "Politics",
  },
  {
    text: "What is Brexit?",
    ...
    correctAnswer: 1,
    explanation: "Brexit refers to the UK's withdrawal from the European Union, which became official on 31 January 2020.",
    topic: "Politics",
  },
];
...
async function loadQuestions(): Promise<Question[]> {
  ...
  if (!apiKey) {
    console.warn("Missing Gemini API key — add VITE_GEMINI_API_KEY to Cloudflare environment variables then redeploy");
    return FALLBACK;
  }
    ...
    const res = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${apiKey}`,
      {
        ...
        body: JSON.stringify({
          contents: [{
            parts: [{
              text: `Generate exactly 5 multiple choice quiz questions about world politics, economics, international relations, and current affairs. Cover different topics — do not repeat the same theme.

Return ONLY a valid JSON array with no other text, no backticks, no explanation:
[
  {
    "text": "Question?",
    "options": ["A", "B", "C", "D"],
...
Be factual, unbiased, and educational.`
            }]
          }],
...
function QuizPage() {
  ...
  useEffect(() => {
    ...
    loadQuestions().then((qs) => {
      if (live) { setQuestions(qs); setLoading(false); }
    });
    return () => { live = false; };
  }, []);
  ...
  const next = () => {
    ...
    if (currentQ + 1 >= questions.length) {
      ...
      const xp = 50 + (newCount === questions.length ? 25 : 0);
    ...
    return (
      <main className="relative flex min-h-screen flex-col items-center justify-center bg-background px-6 text-foreground">
        ...
        <div className="relative w-full max-w-sm text-center">
          <div className="mb-4 text-6xl">{finalScore === 5 ? "🏆" : finalScore >= 3 ? "⭐" : "📚"}</div>
          <h1 className="mb-1 text-6xl font-black text-white">{finalScore} / {questions.length}</h1>
          <p className="mb-8 text-base font-semibold text-white/70">
    ...
    <main className="relative flex min-h-screen flex-col bg-background px-6 pt-12 pb-8 text-foreground">
      ...
      <div className="relative mb-6">
        <div className="mb-2 flex justify-between">
          ...
          <span className="text-sm font-semibold text-white/60">Score: {correctCount}</span>
        </div>
      ...
      {/* Topic */}
      <span className="relative mb-4 inline-block self-start rounded-full border border-white/15 bg-white/10 px-3 py-1 text-sm font-semibold text-white/80">
        {q.topic}
      </span>
      ...
      <div className="relative flex flex-1 flex-col gap-3">
        {q.options.map((opt, idx) => {
          ...
          if (selected === null) {
            cls += "border-white/20 bg-white/8 text-white hover:bg-white/12";
          } else if (idx === q.correctAnswer) {
            cls += "border-green-500 bg-green-500/20 text-green-300 font-bold";
          } else if (idx === selected) {
            cls += "border-red-500 bg-red-500/20 text-red-300 font-bold";
          } else {
