...
/* ─────────────────────────────────────────────
   FALLBACK DEBATES — used when no API key
───────────────────────────────────────────── */
const FALLBACK_DEBATES: DebateData[] = [
  {
    ...
    sideA: {
      ...
      fallbackArgs: [
        "AI systems have already caused documented harm — facial recognition misidentifying innocent people, hiring algorithms discriminating against women, and credit models penalising minorities. These aren't hypothetical risks.",
        "The EU's AI Act demonstrates that meaningful regulation is achievable without banning innovation. Risk-tiered approaches allow light regulation for low-risk systems while applying strict standards where lives are at stake.",
    ...
    sideB: {
      ...
      fallbackArgs: [
        "China and Russia won't regulate their AI development. If Western democracies overregulate, they hand authoritarian states a decisive strategic advantage in the most important technology race of the century.",
        "Regulators consistently lag 10–15 years behind technology. Rules written today govern yesterday's AI while tomorrow's systems operate in a vacuum. We'd be creating compliance theatre, not safety.",
        "Compliance costs primarily benefit Big Tech incumbents who can afford legal teams. Prescriptive regulation systematically disadvantages startups and drives researchers to less restrictive jurisdictions.",
      ],
    },
    baseVotesA: 1847,
    baseVotesB: 1203,
  },
  {
    ...
    sideA: {
      ...
      fallbackArgs: [
        ...
        "Research across 33 countries finds higher social media use correlates consistently with lower institutional trust, greater polarisation, and higher susceptibility to misinformation. This is academic consensus.",
    ...
    sideB: {
      ...
      fallbackArgs: [
        ...
        "Legacy media was also partisan and capable of manufacturing consent. At least social media lets citizens challenge elite narratives in real time rather than passively consuming broadcast media.",
        "The Arab Spring, Iran's Green Movement, and Belarus protest coordination all depended on social media. For millions under authoritarian governments, these platforms are essential, not harmful.",
      ],
    },
    baseVotesA: 2103,
    baseVotesB: 986,
  },
  {
    ...
    sideA: {
      ...
      fallbackArgs: [
        "Finland's 2017–2018 UBI trial found recipients reported significantly higher wellbeing, mental health improvements, and greater trust in institutions — with no reduction in employment.",
        "Automation is already eliminating routine jobs across logistics, manufacturing, and service industries. A basic income provides a transition floor while workers retrain — the alternative is mass insecurity.",
        "UBI would formally recognise unpaid work — caring for children, elderly relatives, volunteering — that currently contributes enormously to society but receives no economic recognition.",
      ],
    },
    sideB: {
      ...
      fallbackArgs: [
        "A UBI set at £1,000 per month for every UK adult would cost approximately £600 billion annually — roughly the entire current government budget. The numbers simply don't add up without catastrophic tax rises.",
        "Evidence from cash transfer programmes consistently shows some recipients reduce working hours. In an already tight labour market, large-scale work reduction would reduce productivity and tax revenues simultaneously.",
        "UBI's universality is its greatest flaw — it gives the same payment to billionaires as to those in genuine need. Targeted welfare spending addresses poverty far more efficiently per pound spent.",
      ],
    },
    baseVotesA: 1542,
    baseVotesB: 1876,
...
/* ─────────────────────────────────────────────
   DAILY DEBATE LOADER — Gemini generates fresh
   debate topic each day, cached until midnight
───────────────────────────────────────────── */
...
async function loadDailyDebate(): Promise<DebateData> {
  ...
  if (!apiKey) {
    /* Rotate through fallbacks by day of year */
    ...
    const res = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${apiKey}`,
      {
        ...
        body: JSON.stringify({
          contents: [{
            parts: [{
              text: `Generate a fresh, timely debate motion about a current political, economic, or social issue. Make it genuinely controversial and interesting for educated 18-28 year olds in the UK/globally.

Return ONLY valid JSON (no markdown, no backticks):
{
  "id": "short-kebab-case-id",
  "topic": "Should X?",
  "context": "2-3 sentences of balanced, factual background context explaining why this is debated.",
  "sideA": {
    "label": "Yes — 3-4 word label",
    "description": "One compelling sentence summarising the case for.",
    "fallbackArgs": [
      "Specific evidence-based argument with a real statistic or example.",
      "Second distinct argument with supporting evidence.",
      "Third argument addressing a different dimension of the issue."
    ]
  },
  "sideB": {
    "label": "No — 3-4 word label",
    "description": "One compelling sentence summarising the case against.",
    "fallbackArgs": [
      "Specific evidence-based counter-argument with real evidence.",
      "Second distinct counter-argument.",
      "Third counter-argument addressing a different concern."
    ]
  },
  "baseVotesA": 1200,
  "baseVotesB": 900
...
Avoid topics used recently. Good topics include: climate policy, immigration, housing, tax, healthcare, education, tech regulation, nuclear energy, drug policy, electoral reform.`,
            }],
          }],
...
/* ─────────────────────────────────────────────
   AI ARGUMENT EXPANDER — adds depth to each side
───────────────────────────────────────────── */
    ...
    const res = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${apiKey}`,
      {
        ...
        body: JSON.stringify({
          contents: [{
            parts: [{
              text: `For the debate: "${topic}"

Write exactly 3 arguments FOR and 3 AGAINST. Each 1-2 sentences, evidence-based and specific.
...
/* ─────────────────────────────────────────────
   HARDCODED COMMUNITY POSTS — per debate id
───────────────────────────────────────────── */
const COMMUNITY_POSTS: Record<string, { id: number; author: string; side: string; time: string; text: string; likes: number }[]> = {
  "ai-regulation": [
    { id: 1, author: "PolicyStudent", side: "A", time: "2h ago", text: "Without regulation, who holds AI companies accountable when systems discriminate in hiring or wrongly flag someone as a criminal? We regulate cars, planes, and drugs — AI can be just as consequential.", likes: 47 },
  ...
  "universal-basic-income": [
    ...
    { id: 2, author: "FiscalRealist", side: "B", time: "4h ago", text: "£600 billion a year. That's what a meaningful UK UBI would cost. Where exactly does that money come from without gutting other public services?", likes: 57 },
  ],
};
...
function DebatePage() {
  ...
  useEffect(() => {
    loadDailyDebate().then((d) => {
      ...
      /* Load AI arguments */
      const fallback: AIArgs = {
        forArgs: d.sideA.fallbackArgs,
        againstArgs: d.sideB.fallbackArgs,
      };
      loadAIArguments(d.topic, d.id, fallback).then((args) => {
        setAiArgs(args);
  ...
  const handlePost = () => {
    ...
    const post = { id: Date.now(), author: "You", side: postSide, time: "Just now", text: newPost.trim(), likes: 0 };
        ...
        {loadingArgs ? (
          ...
          <div className="grid grid-cols-1 gap-3">
            <div className="rounded-3xl border p-5"
              style={{ borderColor: "oklch(0.72 0.18 350 / 0.3)", background: "oklch(0.72 0.18 350 / 0.08)" }}>
              <p className="text-xs font-black uppercase tracking-widest mb-3" style={{ color: "oklch(0.78 0.18 350)" }}>
                ✓ The Case For — {debate.sideA.label}
              </p>
            ...
            <div className="rounded-3xl border border-blue-500/25 bg-blue-500/8 p-5">
              <p className="text-xs font-black uppercase tracking-widest text-blue-300 mb-3">
                ✗ The Case Against — {debate.sideB.label}
              </p>
              <ul className="space-y-2.5">
        ...
        <div className="rounded-3xl border border-white/20 bg-white/8 p-5">
          ...
          <div className="grid grid-cols-2 gap-3">
            <button onClick={() => handleVote("A")}
              className="rounded-2xl border py-4 px-3 text-sm font-black text-left transition-all"
              style={userVote === "A"
                ? { borderColor: "oklch(0.72 0.18 350)", background: "oklch(0.72 0.18 350 / 0.2)", color: "white" }
                : { borderColor: "rgba(255,255,255,0.2)", background: "rgba(255,255,255,0.05)", color: "rgba(255,255,255,0.8)" }}>
              <div className="flex items-center gap-1.5 mb-1">
                <ThumbsUp className="h-3.5 w-3.5" style={userVote === "A" ? { color: "oklch(0.78 0.18 350)" } : {}} />
                <span className="text-xs font-black uppercase tracking-wide"
                  style={userVote === "A" ? { color: "oklch(0.78 0.18 350)" } : { color: "rgba(255,255,255,0.5)" }}>
                  {userVote === "A" ? "✓ Your vote" : "I agree"}
                </span>
              </div>
              {debate.sideA.label}
            </button>
            <button onClick={() => handleVote("B")}
              className="rounded-2xl border py-4 px-3 text-sm font-black text-left transition-all"
              style={userVote === "B"
                ? { borderColor: "rgb(96 165 250)", background: "rgba(96,165,250,0.15)", color: "white" }
                : { borderColor: "rgba(255,255,255,0.2)", background: "rgba(255,255,255,0.05)", color: "rgba(255,255,255,0.8)" }}>
              <div className="flex items-center gap-1.5 mb-1">
                <ThumbsDown className="h-3.5 w-3.5" style={userVote === "B" ? { color: "rgb(147 197 253)" } : {}} />
                <span className="text-xs font-black uppercase tracking-wide"
                  style={userVote === "B" ? { color: "rgb(147 197 253)" } : { color: "rgba(255,255,255,0.5)" }}>
                  {userVote === "B" ? "✓ Your vote" : "I disagree"}
                </span>
              </div>
              {debate.sideB.label}
            </button>
          ...
          {userVote && (
            <p className="mt-3 text-center text-xs font-bold text-white/60">
              +10 XP earned · Change your vote any time
            </p>
          )}
        </div>
            ...
            <div className="rounded-3xl border border-white/20 bg-white/8 p-4 mb-4">
              ...
              <div className="flex gap-2 mb-3">
                <button onClick={() => setPostSide("A")} className="flex-1 rounded-full py-2 text-xs font-black transition-all"
                  style={postSide === "A" ? { background: "oklch(0.72 0.18 350)", color: "white" } : { background: "rgba(255,255,255,0.08)", color: "rgba(255,255,255,0.6)" }}>
                  For
                </button>
                <button onClick={() => setPostSide("B")} className="flex-1 rounded-full py-2 text-xs font-black transition-all"
                  style={postSide === "B" ? { background: "rgb(59 130 246)", color: "white" } : { background: "rgba(255,255,255,0.08)", color: "rgba(255,255,255,0.6)" }}>
                  Against
                </button>
              </div>
              ...
              <div className="flex items-center justify-between mt-2">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={handlePost} disabled={newPost.trim().length < 10}
                    className="rounded-full px-4 py-1.5 text-xs font-black text-white disabled:opacity-40"
                    style={{ background: "oklch(0.72 0.18 350)" }}>
                    Post · +15 XP
                  </button>
                </div>
          ...
          <div className="flex flex-col gap-3" ref={postRef}>
            {posts.map((post) => (
              <div key={post.id} className="rounded-2xl border p-4"
                style={post.author === "You"
                  ? { borderColor: "oklch(0.72 0.18 350 / 0.4)", background: "oklch(0.72 0.18 350 / 0.08)" }
                ...
                <div className="flex items-start justify-between mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    <span className="rounded-full px-2 py-0.5 text-[10px] font-black"
                      style={post.side === "A"
                        ? { background: "oklch(0.72 0.18 350 / 0.2)", color: "oklch(0.78 0.18 350)" }
                        : { background: "rgba(96,165,250,0.2)", color: "rgb(147 197 253)" }}>
