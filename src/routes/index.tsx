...
function Index() {
  ...
  const handleSignIn = () => {
    try {
      const onboarded = localStorage.getItem("civicloop_onboarded");
      if (onboarded === "true") {
        navigate({ to: "/home" });
      } else {
    ...
    <main className="relative flex min-h-screen flex-col items-center justify-between overflow-hidden bg-background px-6 pb-10 pt-20 text-foreground">
      ...
      <div className="relative flex w-full max-w-md flex-1 flex-col items-center justify-center text-center">
        <div className="mb-10 flex items-center gap-2">
          ...
          <span className="text-xs font-black uppercase tracking-[0.2em] text-white/60">CivicLoop</span>
        </div>
        <h1 className="text-balance text-4xl font-black leading-[1.1] tracking-tight sm:text-5xl text-white">
          Understand the world in 5 minutes a day.
        </h1>
        <p className="mt-5 text-base font-bold text-white/75 sm:text-lg">
          Daily quizzes on news, history, and politics.
        </p>
      </div>
      <div className="relative flex w-full max-w-sm flex-col items-center gap-3">
        <button type="button" onClick={handleSignIn}
          className="flex w-full items-center justify-center gap-3 rounded-full border border-white/20 bg-transparent px-5 py-3.5 text-sm font-black text-white transition-colors hover:bg-white/5">
        ...
        <button type="button" onClick={handleSignIn}
          className="mt-1 text-sm font-bold text-white/60 transition-colors hover:text-white">
          Continue as Guest
        </button>
        <p className="mt-8 text-xs font-bold text-white/40">Used by 10,000+ curious minds.</p>
      </div>
    </main>
  );
}
